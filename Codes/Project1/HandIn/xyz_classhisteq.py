...
import numpy as np
import sys
from myModules import class_histogram_equalization
...
# Change color space
image2 = cv2.cvtColor(image1, cv2.COLOR_BGR2XYZ)
# Illumination stretching
image3 = np.copy(image2)
image3[:, :, 1] = class_histogram_equalization(image2[:, :, 1])# Y
# Convert back to BGR
check_range(image3)# check range
image4 = cv2.cvtColor(image3, cv2.COLOR_XYZ2BGR)
